...
list_index=[]
index=0
def data_matching(dict,compare1):
    compare_len=len(str(compare1))
    for key1 in dict.keys():
        length1=len(key1)
        if abs(length1-compare_len)>8:
            arr_dict.pop(key1)
    return arr_dict
...
if __name__ == '__main__':
    ...
    d = sc.textFile('appstore_games.csv',10)
    header=d.first()
    data2=d.filter(lambda row: row!=header).map(lambda x: x.split(','))
    data=data2.map(lambda x: (x[1],x[2])).mapValues(lambda x: x.strip())\
        .mapValues(lambda x: x.lower())\
        .mapValues(lambda x: re.sub('[^a-zA-Z0-9]+','',x))\
        .mapValues(lambda x: get_ngrams(x))\
    ...
    facility_name=pd.read_csv('appstore_games.csv')
    uncluster=[]
    ...
    #print(cluster_dict)
        ...
        #print('')
    f=open('lsh-own-ngram.txt','w+')
    for key in cluster_dict.keys():
        list_out=[]
        for i in cluster_dict[key]:
            list_out.append(facility_name[facility_name['ID'].isin([listOfItemset[i]])]['Name'].values[0])
        arr_dict=dict((x,list_out.count(x)) for x in list_out)
        compare=facility_name[facility_name['ID'].isin([listOfItemset[key]])]['Name'].values[0]
        print(facility_name[facility_name['ID'].isin([listOfItemset[key]])]['Name'].values[0],':  ',sep='',end='',file=f)
        print(facility_name[facility_name['ID'].isin([listOfItemset[key]])]['Name'].values[0],':  ',sep='',end='')
